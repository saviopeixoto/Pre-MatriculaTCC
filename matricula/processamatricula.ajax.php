<?php
//die();
include('config.php'); // incluir quando o get fase esta comentado 
...
function removeAcentos($str){
  $a = array('À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Ç', 'È', 'É', 'Ê', 'Ë','Ì', 'Í', 'Î', 'Ï', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'Ø', 'Ù', 'Ú', 'Û', 'Ü', 'Ý', 'ß', 'à', 'á', 'â', 'ã', 'ä', 'å', 'ç', 'è', 'é', 'ê', 'ë', 'ì', 'í', 'î', 'ï', 'ò', 'ó', 'ô', 'õ', 'ö', 'ø', 'ù', 'ú', 'û', 'ü', 'a', 'A', 'a', 'Ç', 'ç');
  $b = array('A', 'A', 'A', 'A', 'A', 'A','C', 'E', 'E', 'E', 'E', 'I', 'I', 'I', 'I', 'O', 'O', 'O', 'O', 'O', 'O', 'U', 'U', 'U', 'U', 'Y', 'S', 'A', 'A', 'A', 'A', 'A', 'A','C', 'E', 'E', 'E', 'E', 'I', 'I', 'I', 'I', 'O', 'O', 'O', 'O', 'O', 'O', 'U', 'U', 'U', 'U', 'A', 'A', 'A', 'C', 'C');
  return strtoupper(str_replace($a, $b, $str));
}
//private function removeAcentos ($string) {
	//	return preg_replace(array("/(á|à|ã|â|ä)/", "/(Á|À|Ã| |Ä)/","/(é|è|ê|ë)/","/(É|È|Ê|Ë)/","/(í|ì|î|ï)/","/(Í|Ì|Î|Ï)/","/(ó|ò|õ|ô|ö)/","/(Ó|Ò|Õ|Ô|Ö)/","/(ú|ù|û|ü)/","/(Ú|Ù|Û|Ü)/","/(ñ)/","/(Ñ)/","/(ç)/","/(Ç)/"), explode(" ", "a A e E i I o O u U n N c C"), $string);
	//}

function phonebr($text, &$result) {
   ...
   for($i=0;$i<strlen($text);$i++) { 
      switch($text[$i]) { 
         ...
         case 'C':
            if ($text[$i+1] == H) {
               $result .= X;           
            } elseif(($text[$i+1] == E) or ($text[$i+1] == I)) {
               $result .= SS;
            } else {
               $result .= K;
            }
         ...
         case 'S': 
            ...
            if ($text[$i+1] == H) {
               $result .= X;
            } elseif (($text[$i-1] == S) or ($text[$i+1] == S) or (!in_array($text[$i+1], $vog)) ) {
                  $result .= S;
            } elseif (($i == 0) or (!in_array($text[$i+1], $vog))) {
                  $result .= SS;
            } else {
                  $result .= Z;
            }
...
//TODO
//if($mo01_idadecorte > 3) die('idade corte fechada'); // ja mudei aqui
$fase_temporario = '18'; //get_fase();   //$fase_temporario = ($mo01_idadecorte > 3) ? get_fase() : '11';
...
$mo01_gemeo         = $_POST['mo01_gemeo'];
$mo01_creche        = $_POST['mo01_creche'];

//=============================================

if(!in_array($mo01_gemeo, [0, 1])) $mo01_gemeo = 0;

if(!in_array($mo01_creche, [0, 1])) $mo01_creche = 0;

//=============================================

$mo01_cpfresp  = preg_replace('/[^0-9]/', '', $mo01_cpfresp);
if(!is_numeric($mo01_cpfresp)) die('codigo invalido');

$arr_fases_consulta = implode("','", get_arr_fases_consulta());

$sql = <<<EOF
	SELECT *, sme01_statusid
	  FROM plugins.mobase
 LEFT JOIN matriculaonline.complemento01 ON sme01_mobaseid = mo01_codigo
	   AND mo01_fase IN (17, 18)
	   AND (sme01_statusid IS NULL OR sme01_statusid IN (5, 6, 7))
...
while($row = pg_fetch_array($result, null, PGSQL_ASSOC)) {
	if($row['mo01_nome'] == $mo01_nome) $flag = true;
}
/*
$cpfresp = str_replace('.', '', $mo01_cpfresp);
$cpfresp = str_replace('-', '', $cpfresp);

$sSqlCadastro  = "select mo01_nome as nome from plugins.mobase Where mo01_tipo = 'P2020' and mo01_cpfresp = '".$cpfresp."'";    

$rsCadastro = db_query($sSqlCadastro);
$registros  = pg_num_rows($rsCadastro);

$mo01_nome = caixaAlta($mo01_nome);
phonebr($mo01_nome,$nomedigitado);

$flag = false;
if ($registros<=0) {
   if (!is_string($mo01_nome)) {
      $flag = true;
   }
} else {
   if (!is_string($mo01_nome)) {
      ...
      for($cont=0;$cont<$registros;$cont++) {
         ...
         phonebr($nome,$nomebanco);
         if ($nomebanco == $nomedigitado) {
            $flag = true;
            break;
         }
      }
...
$nomepesq = caixaAlta($mo01_nome);

$flag = false;
if ($registros<=0) {
   if (!is_string($nomepesq)) {
      $flag = true;
   }
} else {
   if (!is_string($nomepesq)) {
      ...
      for($cont=0;$cont<$registros;$cont++) {
         db_fieldsmemory($rsCadastro,$cont);   
         if (is_string($nomepesq)) {
            similar_text(strtolower($nome),strtolower($nomepesq),$percent);  
            if ($percent==100) {
               $flag = true;
               break;
            }
...
$flag = false; //rodrigodevolder@gmail.com
   ...
   if (!empty($mo01_certidaomatricula)) {
    ...
    try {
      ...
      if (!empty($mo01_nomebair)) {
         $mo01_nomebair = removeAcentos($mo01_nomebair);
      ...
      if (!empty($mo01_nomeend) && $mo01_ender == "") {
         $mo01_nomeend = removeAcentos($mo01_nomeend);
      ...
      $clmobase->mo01_nome          = removeAcentos($mo01_nome);
      $clmobase->mo01_sexo          = $mo01_sexo;
      $clmobase->mo01_cor           = $mo01_cor;
      $clmobase->mo01_estciv        = $mo01_estciv;
      $clmobase->mo01_nacion        = removeAcentos($mo01_nacion);
      $clmobase->mo01_ufnasc        = $mo01_ufnasc;
      $clmobase->mo01_munnasc       = $mo01_munnasc;
      $clmobase->mo01_mae           = removeAcentos($mo01_mae);
      $clmobase->mo01_pai           = removeAcentos($mo01_pai);
      $clmobase->mo01_tiporesp      = $mo01_tiporesp;
      $clmobase->mo01_nomeresp      = removeAcentos($mo01_nomeresp);
      ...
      $clmobase->mo01_tipo          = 'P2023';
      $clmobase->mo01_fase          = '18'; //$fase_temporario;
	  $clmobase->mo01_gemeo         = $mo01_gemeo;
	  $clmobase->mo01_creche        = $mo01_creche;
      $clmobase->incluir(null);

      if ($clmobase->erro_status == 0) {
         throw new DBException( $clmobase->erro_msg);
      }

      $mo01_codigo = $clmobase->mo01_codigo;
      ...
      $data_c = explode('-',$clmobase->mo01_datacad);
      //$clmobase->mo01_protocol = $mo01_codigo.$data_c[1].$data_c[0];
	  $clmobase->mo01_protocol = $mo01_codigo;
      $clmobase->alterar($mo01_codigo);
      ...
      db_fim_transacao();
	  $mo01_protocol = $clmobase->mo01_protocol;
      $oRetorno->mo01_codigo = $mo01_codigo;
      $oRetorno->mo01_protocol = $mo01_protocol;
      $oRetorno->mo01_nome = removeAcentos($mo01_nome);
      $oRetorno->mo01_nomeresp = removeAcentos($mo01_nomeresp);
      $oRetorno->mo01_cpfresp = $mo01_cpfresp;
      $oRetorno->sMessage = 'Inscrição efetuada com Sucesso!';
      echo $oJson->encode($oRetorno);
